...
function MakeDocsHomePage {
    ...
    if ($namespaces.length -ne 0) {
        ...
        # The navbar link automatically selects the first namespace, but there's no way
        # to do that from within the home page text, so the best we can do is to tell
        # people to click the navbar link.
        $markdown += "`n`nClick **API Documentation** above to see all namespaces and types."
    }
