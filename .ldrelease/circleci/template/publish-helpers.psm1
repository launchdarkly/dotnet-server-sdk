...
function PreparePackagesToPublish {
    ...
    if ($shouldSign) {
        ...
        copy-item -path "${env:LD_RELEASE_SECRETS_DIR}/dotnet_code_signing_certificate_pkcs12" -destination $certFile
        $certPassword = get-content -path "${env:LD_RELEASE_SECRETS_DIR}/dotnet_code_signing_private_key_passphrase" -raw
    }
