...
namespace LaunchDarkly.Sdk.Server.Internal.Evaluation
{
    public class BucketingTest
    {
        ...
        public void VariationIndexForBucket()
        {
            ...
            var bucketValue = (int)(Bucketing.BucketUser(null, user, flagKey, null, salt) * 100000);
            ...
            var rollout = new Rollout(RolloutKind.Rollout, null, null, variations, null);
        ...
        public void UsingSeedIsDifferentThanSalt()
        {
            ...
            var bucketValue1 = Bucketing.BucketUser(null, user, flagKey, null, salt);
            var bucketValue2 = Bucketing.BucketUser(seed, user, flagKey, null, salt);
        ...
        public void DifferentSeedsProduceDifferentAssignment()
        {
            ...
            var bucketValue1 = Bucketing.BucketUser(seed1, user, flagKey, null, salt);
            var bucketValue2 = Bucketing.BucketUser(seed2, user, flagKey, null, salt);
            Assert.NotEqual(bucketValue1, bucketValue2);
        }
        ...
        public void FlagKeyAndSaltDoNotMatterWhenSeedIsUsed()
        {
            ...
            var bucketValue1 = Bucketing.BucketUser(seed, user, flagKey1, null, salt1);
            var bucketValue2 = Bucketing.BucketUser(seed, user, flagKey2, null, salt2);
        ...
        public void LastBucketIsUsedIfBucketValueEqualsTotalWeight()
        {
            ...
            int bucketValue = (int)(Bucketing.BucketUser(null, user, flagKey, null, salt) * 100000);
        ...
        public void CanBucketByIntAttributeSameAsString()
        {
            var user = User.Builder("key")
                ...
                .Build();
            var resultForString = Bucketing.BucketUser(null, user, "key", "stringattr", "salt");
            var resultForInt = Bucketing.BucketUser(null, user, "key", "intattr", "salt");
        ...
        public void CannotBucketByOtherDataTypes()
        {
            ...
            })
            {
                var user = User.Builder("key")
                    ...
                    .Build();
                var result = Bucketing.BucketUser(null, user, "key", "badattr", "salt");
                if (result != 0f)
                {
        ...
        public void UserSecondaryKeyAffectsBucketValue()
        {
            ...
            var result1 = Bucketing.BucketUser(null, user1, flagKey, null, salt);
            var result2 = Bucketing.BucketUser(null, user2, flagKey, null, salt);
            ...
            )
        {
            ...
            var flag2 = new FeatureFlagBuilder(flagKey)
                ...
                .Rules(new RuleBuilder().Id("id")
                    .Rollout(rollout)
                    .Clauses(new ClauseBuilder().Attribute("key").Op(Operator.In).Values(LdValue.Of(user.Key)).Build())
                    .Build())
                .Salt(salt)
                .Build();
